...
case class CashStatementForEori(
...
case class CashStatementMonthToMonth(
  startDate: LocalDate,
  endDate: LocalDate,
  files: Seq[CashStatementFile] = Seq.empty
)(implicit messages: Messages)
    extends Ordered[CashStatementMonthToMonth] {

  val formattedMonthToMonth: String = files.headOption
    .map { file =>
...
case class GroupedStatementsByEori(
  eoriIndex: Int,
  eoriHistory: EoriHistory,
...
object CashStatementViewModel {
  ...
  def generateStatementsByYear(groupedStatements: GroupedStatementsByEori)(implicit messages: Messages): Html = {
    val statementRows = groupedStatements.statementsByYear.toSeq
      .sortBy(_._1)
      .map { case (year, statementsOfYear) =>
        val statementList = createStatementList(groupedStatements.eoriIndex, statementsOfYear)
        val yearHeading   =
          h2Component(msg = year.toString, classes = "govuk-heading-s govuk-!-margin-bottom-0 govuk-!-margin-top-7")
  ...
  private def createStatementList(eoriIndex: Int, statementsOfYear: Seq[CashStatementMonthToMonth])(implicit
    messages: Messages
  ): Html = {
    val statementListItems = statementsOfYear.sorted.zipWithIndex.map { case (statement, index) =>
      createStatementRow(eoriIndex, statement, index).body
    }.mkString
  ...
  private def createStatementRow(eoriIndex: Int, statement: CashStatementMonthToMonth, index: Int)(implicit
    messages: Messages
  ): Html = {
  ...
  private def createDownloadLinks(eoriIndex: Int, statement: CashStatementMonthToMonth, index: Int)(implicit
    messages: Messages
  ): String = {
  ...
  private def downloadLink(file: Option[CashStatementFile], format: String, id: String, period: String)(implicit
    messages: Messages
  ): Html =
    file match {
      case Some(f) =>
        new download_link_cash_account()
          .apply(file = Some(f), fileFormat = FileFormat(format), id = id, period = period)
