  ...
  "ErrorSummary component" must {
    ...
    "show correct error with unchanged key when isErrorKeyUpdateEnabled is false" in new SetUp {
      val errorSum: ErrorSummary = ErrorSummary(
        errorList = Seq(
          ErrorLink(
            Some("#start"),
            content = Text(msgs("cf.historic.document.request.form.error.end.year.date-number-invalid"))
          )
        ),
      ...
      val formErrors: Seq[FormError]    =
        Seq(FormError("start", "cf.historic.document.request.form.error.end.year.date-number-invalid"))
      val result: HtmlFormat.Appendable = view(formErrors, None)

      result
        .toString()
        .contains(
          "<a href=\"#start\">cf.historic.document.request.form.error.end.year.date-number-invalid</a>"
        )                                               shouldBe true
      ...
      "updateFormErrorKeyForTheMessage function is provided" in new SetUp {
        ...
        val errorSum: ErrorSummary = ErrorSummary(
          errorList = Seq(
            ErrorLink(
              Some("#start.month"),
              content = Text(msgs("cf.historic.document.request.form.error.start.month.invalid"))
            )
          ),
          title = Text(msgs("error.summary.title"))
        )
        ...
        val view: errorSummary            = app.injector.instanceOf[errorSummary]
        val formErrors: Seq[FormError]    =
          Seq(FormError("start", "cf.historic.document.request.form.error.start.month.invalid"))
        ...
        result
          .toString()
          .contains(
            "<a href=\"#start.month\">cf.historic.document.request.form.error.start.month.invalid</a>"
          )                                               shouldBe true
        result.toString().contains("error.summary.title") shouldBe true
      }
    ...
    "show correct error with updated key when key has value start,error msg key is cf.historic.document.request.form.error.year.invalid " +
      " isErrorKeyUpdateEnabled is true and updateFormErrorKeyForTheMessage function is provided" in new SetUp {
        ...
        val errorSum: ErrorSummary = ErrorSummary(
          errorList = Seq(
            ErrorLink(Some("#start.year"), content = Text(msgs("cf.historic.document.request.form.error.year.invalid")))
          ),
          title = Text(msgs("error.summary.title"))
        )
        ...
        result
          .toString()
          .contains("<a href=\"#start.year\">cf.historic.document.request.form.error.year.invalid</a>") shouldBe true
        result.toString().contains("error.summary.title")                                               shouldBe true
      }
    ...
    "show correct error with updated key when key has value end,error msg key is cf.historic.document.request.form.error.year.invalid " +
      " isErrorKeyUpdateEnabled is true and updateFormErrorKeyForTheMessage function is provided" in new SetUp {
        ...
        val errorSum: ErrorSummary = ErrorSummary(
          errorList = Seq(
            ErrorLink(Some("#end.year"), content = Text(msgs("cf.historic.document.request.form.error.year.invalid")))
          ),
          title = Text(msgs("error.summary.title"))
        )
        ...
        result
          .toString()
          .contains("<a href=\"#end.year\">cf.historic.document.request.form.error.year.invalid</a>") shouldBe true
        result.toString().contains("error.summary.title")                                             shouldBe true
      }
  }
